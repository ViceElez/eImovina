...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy(
        ClassAuthorizationPolicies.AdminOnly,
        policy => policy.RequireRole("Admin"));

    options.AddPolicy(
        ClassAuthorizationPolicies.ManageEquipment,
        policy => policy.RequireRole("Admin", "InventoryManager"));

    options.AddPolicy(
        ClassAuthorizationPolicies.ViewEquipment,
        policy => policy.RequireRole("Admin", "InventoryManager", "LocationResponsible"));

    options.AddPolicy(
        ClassAuthorizationPolicies.EmployeeAccess,
        policy => policy.RequireRole("Admin", "InventoryManager", "LocationResponsible", "Employee"));
});
